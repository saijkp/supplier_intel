...
CREATE TABLE IF NOT EXISTS suppliers (
    ...
    catalogue_depth_extracted_at   TIMESTAMP,    -- set on every attempt, matched or not -- same discipline as capability_extracted_at/contacts_found_at/factory_facts_extracted_at

    -- Reverse-image-search evidence (v25) -- see batch/ris_importer.py. Sourced
    -- from a one-time import of an external local geocode/Street-View/RIS
    -- pipeline's own output (not generated by this codebase), same plain-column
    -- shape as companies_house_* since these are single per-supplier values, not
    -- a one-to-many table like supplier_capabilities/supplier_catalogue_signals.
...
-- ═══════════════════════════════════════
-- Catalogue-depth evidence (v24): customer-logos section / named case
-- studies / specific process-machine detail, extracted from a
-- supplier's own already-collected website pages by
-- verification.catalogue_depth_extractor.CatalogueDepthExtractor.
-- Evidence for a human to weigh manually when answering the buyer
-- tracker's own criterion A (Website Deep-Dive) -- never a verdict on
-- it, same discipline as every other evidence table in this schema
-- (supplier_capabilities, field_provenance). A separate table from
-- supplier_capabilities rather than a new category there: these
-- signals aren't vocabulary-mapped free-text claims (see
-- capability_vocabulary.py's fixed CATEGORY_* set) and have no
-- in_house/subcontracted/asserted relationship to record.
-- ═══════════════════════════════════════
CREATE TABLE IF NOT EXISTS supplier_catalogue_signals (
    ...
    -- Deliberately NOT unique on (supplier_id, signal_type) alone --
    -- unlike a capability claim, a supplier can genuinely have several
    -- distinct pieces of evidence for the same signal (e.g. two named
    -- case studies on two different pages), and collapsing them to one
    -- row would silently discard real evidence.
);
...
MIGRATIONS: dict[int, dict] = {
    ...
    24: {
        "description": (
            "Catalogue-depth evidence (opt-in, manually invoked -- see "
            "verification/catalogue_depth_service.py): supplier_catalogue_signals "
            "(customer_logos_section / named_case_studies / specific_process_detail, "
            "each with an evidence quote and source_url, extracted from a supplier's "
            "own already-collected website pages -- no new fetch) plus "
            "suppliers.catalogue_depth_extracted_at (idempotency marker, set on "
            "every attempt). Evidence for the buyer tracker's own criterion A "
            "(Website Deep-Dive), never a verdict on it."
        ),
        "columns": [
            ("suppliers", "catalogue_depth_extracted_at", "TIMESTAMP"),
        ],
        "statements": [
            """
            CREATE TABLE IF NOT EXISTS supplier_catalogue_signals (
    ...
    25: {
        "description": (
            "Reverse-image-search evidence (one-time import, manually invoked -- "
            "see batch/ris_importer.py and main.py import-ris-findings): "
            "suppliers.ris_verification_flag/ris_exact_duplicate_domains/"
            "ris_other_matching_domains, sourced from an external local geocode/"
            "Street-View/reverse-image-search pipeline's own spreadsheet output "
            "(not generated by this codebase -- there is no live RIS pipeline "
            "here), plus ris_imported_at recording when the one-time import wrote "
            "each row. Evidence only, same never-a-verdict discipline as every "
            "other evidence field in this schema."
        ),
        "columns": [
            ("suppliers", "ris_verification_flag", "TEXT"),
