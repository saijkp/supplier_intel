...
class TestSingleItemHardTimeout:
    """SiteCollector's own launch()/new_context()/new_page() calls have
    no timeout of their own (unlike page.goto(), which has
    COLLECTION_PAGE_TIMEOUT_MS) -- real incident: a real batch-upload
    job hung mid-file with zero progress and zero exception/crash trace
    for 13+ minutes, confirmed live via its own deploy logs (real
    activity for 7 suppliers, then total silence starting the 8th's
    browser launch). _collect_with_hard_timeout is the fix: a bounded
    wait via a background thread, not a direct call trusted to always
    return. FakeSiteCollector's delay_seconds stands in for "hangs
    indefinitely" here -- these tests use a delay comfortably longer
    than the configured timeout, not an actual infinite hang, so the
    suite itself never blocks."""

    def test_a_slow_collect_call_times_out_and_returns_a_failed_outcome(self, repo):
        supplier_id = repo.create_golden_record({"canonical_name": "Acme", "domain": "acme.example.com"})
        fake = FakeSiteCollector(delay_seconds=5)
        service = CollectionService(repo=repo, site_collector=fake, single_item_timeout_seconds=1)

        outcome = service.collect(supplier_id)  # must not block for the full 5s delay
    ...
    def test_a_timed_out_collect_call_still_releases_the_browser_semaphore(self, repo):
        """The semaphore `with` block in _collect_one wraps the whole
        _collect_with_hard_timeout call, not the inner daemon thread --
        giving up waiting must still free the permit, or a single hang
        would permanently starve every other caller of a browser-launch
        slot, not just block this one call."""
        supplier_id = repo.create_golden_record({"canonical_name": "Acme", "domain": "acme.example.com"})
        fake = FakeSiteCollector(delay_seconds=5)
        service = CollectionService(repo=repo, site_collector=fake, single_item_timeout_seconds=1)
        ...
        assert _BROWSER_SEMAPHORE._value == COLLECTION_MAX_CONCURRENT_BROWSERS

    def test_a_call_finishing_within_budget_is_unaffected(self, repo):
        supplier_id = repo.create_golden_record({"canonical_name": "Acme", "domain": "acme.example.com"})
        fake = FakeSiteCollector(delay_seconds=0.05)
        service = CollectionService(repo=repo, site_collector=fake, single_item_timeout_seconds=30)

        outcome = service.collect(supplier_id)

        assert outcome["status"] == "success"

    def test_site_collector_raising_inside_the_timeout_wrapper_still_propagates(self, repo):
        """Regression guard: an exception raised inside the background
        thread must reach the calling thread's own exception handling
        in _collect_one (same "SiteCollector never raises by contract,
        but caught anyway as defence in depth" behaviour
        test_site_collector_raising_is_caught_not_propagated already
        covers) -- it must never be silently lost just because it
        happened inside a thread this method spawned and is waiting on."""
        supplier_id = repo.create_golden_record({"canonical_name": "Acme", "domain": "acme.example.com"})
        fake = FakeSiteCollector(raise_error=RuntimeError("browser crashed"))
        service = CollectionService(repo=repo, site_collector=fake, single_item_timeout_seconds=30)

        outcome = service.collect(supplier_id)  # must not raise, must not lose the real error

        assert outcome["status"] == "failed"
        assert "browser crashed" in outcome["error"]

    def test_default_timeout_is_the_configured_settings_constant(self, repo):
        from config.settings import COLLECTION_SINGLE_ITEM_TIMEOUT_SECONDS

        service = CollectionService(repo=repo, site_collector=FakeSiteCollector())
        assert service.single_item_timeout_seconds == COLLECTION_SINGLE_ITEM_TIMEOUT_SECONDS
