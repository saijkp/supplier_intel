...
`frontend/index.html` (the live production frontend, `https://grand-alfajores-a9d7ee.netlify.app/`) is the four-tab evidence-rich app (Find Suppliers, Search, Bulk enrichment, Job History, Compare, Audit — see FRONTEND.md's own description) — single-file vanilla-JS, no framework, no build step. This is the version with real, tested fixes (deep-collect checkbox correctly hidden for a company-name/URL search since that path always collects unconditionally regardless of the checkbox; a free evidence-only Satellite view link next to Street View in the Audit tab; the Enrich/Extract-factory-facts buttons shared between the Search tab and Find Suppliers' own results screen via one `coreEnrichButtonsHtml()` helper, not two copies).

`frontend/console.html` is a newer "Supplier Intel Console" dashboard UI (sidebar nav across Dashboard/Find suppliers/Pipeline Jobs/Buyer Profiles/Settings, backed by `GET /dashboard/summary`) that was briefly the deployed default (2026-09-14 to 2026-09-16) before being reverted here in favour of the tested four-tab app above — preserved intact, not deleted, reachable at `/console.html`, in case that work is picked back up and connected later. `frontend/dashboard.html` is kept as an identical copy of `console.html` (not of `index.html`) at `/dashboard.html` — edit those two together if the console work resumes.
