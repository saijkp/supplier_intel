...
- **`discovery/`** — `DiscoveryService`: AI-assisted supplier discovery grounded in real SerpAPI search results (never hallucinated company names), gated by real fetch/name-match/trader checks in `discovery/candidate_validator.py` before a candidate is stored.
