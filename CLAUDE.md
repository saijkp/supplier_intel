...
`frontend/index.html` (the live production frontend, `https://grand-alfajores-a9d7ee.netlify.app/`) is the "Supplier Intel Console" dashboard UI — single-file vanilla-JS, no framework, no build step, sidebar nav across five pages (`data-page` nav buttons + matching `<section id="page-...">`): Dashboard (backed by `GET /dashboard/summary`, with drill-through cards into filtered Suppliers/Pipeline views), Suppliers (search, with a row-click detail modal via `GET /suppliers/{id}` showing AI summary/contacts/capabilities/certificates), Pipeline Jobs, Buyer Profiles, Settings (API connection). `frontend/dashboard.html` is kept as an identical copy at `/dashboard.html`, same file content as `index.html` — edit both together, they're expected to drift only if one is intentionally forked. `frontend/legacy.html` is the OLDER four-tab app (Search, Discover & enrich, Source, Compare — see FRONTEND.md's description, which still documents *that* UI) — kept for reference/rollback, no longer the deployed default, not actively maintained going forward.
