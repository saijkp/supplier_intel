...
`frontend/index.html` is a single-file vanilla-JS app (no framework, no build step) with four tabs (`data-view` nav buttons + matching `<section id="view-...">`): Search, Discover & enrich, Source (the sourcing-agent chat), Compare (7-dimension supplier ranking, computed client-side from already-returned fields — see `RANK_DIMENSIONS` — never a new server-side score).
